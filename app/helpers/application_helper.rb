...
module ApplicationHelper
  ...
  def avatar(*attrs)
    ...
    if user
      tag.span(class: "avatar", style: options[:size] ? "width: #{options[:size]}px; height: #{options[:size]}px;" : "") do
      ...
      tag.span(class: "avatar", style: "width: #{options[:size]}px; height: #{options[:size]}px;") do
        ""
      end
    end
  end
  ...
  def map(canteens, options = {})
    ...
    tag.div(nil, class: "map", id: (options[:id] || "map"), data: {map: (options[:id] || "map"), markers: markers.to_json, hash: options[:hash]})
  end
