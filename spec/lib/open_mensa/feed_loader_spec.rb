...
describe OpenMensa::FeedLoader do
  ...
  describe "#load!" do
    it "loads data from canteen URL" do
      expect(loader.load!.read).to eq(mock_content("canteen_feed.xml"))
    end

    it "follows temporary redirects (1)" do
      feed.url = "http://example.com/307.xml"
      expect(loader.load!.read).to eq("<xml>")
    end

    it "follows temporary redirects (2)" do
      feed.url = "http://example.com/302.xml"
      expect(loader.load!.read).to eq("<xml>")
    end

    it "follows temporary redirects (3)" do
      feed.url = "http://example.com/303.xml"
      expect(loader.load!.read).to eq("<xml>")
    end

    it "follows temporary redirects for a maximum of 2 redirect by default" do
    ...
    it "follows update canteen URL on permanent redirect" do
    ...
    it "raises an error on socket error" do
